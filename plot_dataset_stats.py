...
# Mapping kategori (exclude id=0 'sampah' yang merupakan super-class)
CATEGORY_NAMES = {
    1: 'Cloth',
    2: 'Foam',
    3: 'Hard Plastic',
...
for split in SPLITS:
    ...
    # Hitung jumlah anotasi per kategori (skip id=0)
    cat_counter = collections.Counter()
    for ann in annotations:
        cid = ann['category_id']
        if cid == 0:
            continue
        cat_counter[categories[cid]] += 1
    ...
    split_img[split]    = {
        ...
        'labeled'  : labeled,
        'unlabeled': unlabeled,
        'anns'     : len([a for a in annotations if a['category_id'] != 0]),
    }
